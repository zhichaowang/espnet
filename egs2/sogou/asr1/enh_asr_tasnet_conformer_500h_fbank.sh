...
inference_args="--normalize_output_wav true "  # Arguments for decoding, e.g., "--lm_weight 0.1".
...
if [ -z "${inference_tag}" ]; then
    ...
    inference_tag+="_asr_model_$(echo "${decode_joint_model}" | sed -e "s/\//_/g" -e "s/\.[^.]*$//g")"
fi

# The directory used for collect-stats mode
joint_stats_dir="${expdir}/asr_stats"
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # prepare train and valid data parameters
        _train_data_param="--train_data_path_and_name_and_type ${_joint_train_dir}/${_scp},speech_mix,${_type} "
        _train_data_param+="--train_data_path_and_name_and_type ${_joint_train_dir}/text,text_ref1,text "
        _train_shape_param="--train_shape_file ${joint_stats_dir}/train/speech_shape "
        _train_shape_param+="--train_shape_file ${joint_stats_dir}/train/text_shape "
        _valid_data_param="--valid_data_path_and_name_and_type ${_joint_valid_dir}/${_scp},speech_mix,${_type} "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_joint_valid_dir}/text,text_ref1,text "
        _valid_shape_param="--valid_shape_file ${joint_stats_dir}/valid/speech_shape "
        _valid_shape_param+="--valid_shape_file ${joint_stats_dir}/valid/text_shape "
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            _dir="${joint_exp}/decode_${dset}_${inference_tag}"
            _logdir="${_dir}/logdir"
            mkdir -p "${_logdir}"
        ...
        for dset in ${test_sets}; do
            ...
            for _type in cer wer ter; do
                ...
                for spk in $(seq "${spk_num}"); do
                    ...
                    paste \
                       <(<"${_dir}/token_spk${spk}" awk -F" " '{for(i=2; i<=NF; i++)printf("%s ", $i);print ""}')  \
                       <(<"${_dir}/text_spk${spk}" awk '{print "(" $1 ")"}')  \
                       >"${_scoredir}/hyp.trn"
                done
...
#                    --results-dir ${_scoredir}
                ...
                score_sclite_espnet2_sogou.sh ${_scoredir}
#                sclite \
