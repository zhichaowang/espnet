...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_mix_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            for spk in $(seq "${spk_num}"); do
                _opts+="--train_data_path_and_name_and_type ${_split_dir}/text_spk${spk},text_ref${spk},text "
            done
            ...
            for spk in $(seq "${spk_num}"); do
                _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_spk${spk},text_ref${spk},text "
            done
